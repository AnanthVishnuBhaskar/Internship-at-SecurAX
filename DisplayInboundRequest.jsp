...
{
	 //alert(id);
	 document.getElementById('inbound_request_id').value=id;
	 document.getElementById('request').value='edit_inbound_request';
	 //alert(document.getElementById('request').value);
	 
	document.getElementById('editInboundRequest').submit();
...
function setIdForDelete(id)
{
	...
	{
		     document.getElementById('inbound_id_delete').value=id;//item_id_delete
			//alert(deleteForm.item_id_delete.value);
		     deleteInboundRequestForm.request.value='delete_inbound_request';
			 document.getElementById('deleteInboundRequestForm').submit();
    } 
	else 
	{
...
%>
 ...
 List<Inventory_Inbound> list = (List)session.getAttribute("inbound_request_display");

%>


<%
for(Inventory_Inbound details: list)
{
	 editableStatus=details.getINBOUND_EDITABLE_STATUS();
}
...
<body class="hold-transition skin-blue sidebar-mini">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1><% 
for(Inventory_Inbound req:list)
{
	%>
      ...
      <ol class="breadcrumb">
        ...
        <% if(editableStatus.equalsIgnoreCase("TRUE") || editableStatus==null)
        		{
        	
        	%>	
        <button style="background-color: #2FA3E6;width: 50px;height: 30px;color: white;border: #1E95DA"  onclick="setId('<%=req.getINBOUND_ID()%>')">Edit
        </button>
        
        <% }%>
 <button style="background-color: #CE1A01;width: 50px;height: 30px;color: white;border: #1E95DA" onclick="setIdForDelete('<%=req.getINBOUND_ID()%>');">Delete</button>
</li> 
      </ol> <%} %>
    </section>
...
<script src="dist/js/demo.js"></script>
<form id="editInboundRequest" method="post" action="Controller">
  
  <input type="hidden" name="inbound_request_id" id="inbound_request_id"> 
 <input type="hidden" name="request" id="request"> 
  
  
  </form>
  <form id="deleteInboundRequestForm" method="post" action="Controller">
  
  <input type="hidden" name="inbound_id_delete" id="inbound_id_delete"> 
 <input type="hidden" name="request" id="request"> 
