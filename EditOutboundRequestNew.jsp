...
function callPrint()
{
	 ...
	 window.open('OutboundInvoiceTable.jsp', '_blank');
...
{
	// alert(id);
	 document.getElementById('outbound_request_id').value=id;
	 document.getElementById('request').value='edit_outbound_request';
...
%>
 ...
 List<Inventory_Outbound_New> list = (List)session.getAttribute("edit_single_ob");
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <%-- <button style="background-color: #2FA3E6;width: 50px;height: 30px;color: white;border: #1E95DA"  onclick="setId('<%=7%>')">Edit</button>
...
REMARK
  ...
  <td width="200px">
  <textarea rows="5" cols="20" name="remark"  class="inputs" autofocus="autofocus">
  
 <%=req.getRemark().trim()%>
  </textarea>
  
   
  <input type="hidden" name="outbound_id" value=" <%=req.getOutbound_id()%>" class="inputs">
  ...
  <input type="hidden"   name="request" value="outbound_update">
   <input type="submit" value="UPDATE" class="buttonClass">
  </td>
  </tr>
</table>
...
<script src="dist/js/demo.js"></script>
<form id="editOutboundRequest" method="post" action="Controller">
  
  <input type="hidden" name="outbound_request_id" id="outbound_request_id"> 
