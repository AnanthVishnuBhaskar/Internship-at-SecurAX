...
function setId(id)
{
	 ...
	 document.getElementById('request').value='edit_user';
...
function setIdForDelete(id)
{
	...
	{
		     document.getElementById('user_id_delete').value=id;//item_id_delete
			//alert(deleteForm.item_id_delete.value);
		     deleteUserForm.request.value='delete_user';
...
List<Inventory_Users> list=(List)session.getAttribute("user_single");
...
for(Inventory_Users user:list)
{
	%>
      ...
      <ol class="breadcrumb">
        ...
        <li class="active"><button style="background-color: #2FA3E6;width: 50px;height: 30px;color: white;border: #1E95DA"  onclick="setId('<%=user.getUser_id()%>')">Edit</button>
 <button style="background-color: #CE1A01;width: 50px;height: 30px;color: white;border: #1E95DA" onclick="setIdForDelete('<%=user.getUser_id()%>');">Delete</button>
</li> 
      </ol> <%} %>
    </section>
...
<script src="dist/js/demo.js"></script>
<form id="editUserForm" method="post" action="Controller">
 ...
 <input type="hidden" name="request" id="request"> 
  ...
  <form id="deleteUserForm" method="post" action="Controller">
