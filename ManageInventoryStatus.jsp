...
{
	 //alert(data);
        window.document.location = 'AddInventoryStatus.jsp';
 ...
 function setId(id)
 {
	 ...
	 document.getElementById('request').value='status_display';
...
List<Inventory_Status> list=dao.getAllInventoryStatus();
...
<div class="wrapper">
  ...
  <form action="Controller" method="post" id='displayStatusForm'>  
 ...
 <thead>
    ...
    <td>Status Type
     ...
     <td>IS_COMPLETED_STATE
    </td>
       
    </tr></thead>
     
    <tbody>  <%
    for(Inventory_Status status:list)
{
...
<tr style="cursor: pointer;"  onclick="setId('<%=status.getSTATUS_ID() %>');">

  
   
 <td width="200px"><%=status.getSTATUS_NAME()%></td>
  <td width="200px"><%=status.getSTATUS_TYPE()%></td>
  <td width="200px"><%=status.getSTATUS_AVAILABILITY() %></td>
